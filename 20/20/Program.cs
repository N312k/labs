...
public class Var
{
    static void Main(string[] args)
    {
        ...
        string pattern = @"(double|int|float|string|var|object|T) \S* ?(?:=) ?(\S)+?(?=;)";
        string path = "D:/input2.txt";
        ...
        while (line != null)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                {
                    Console.WriteLine($"Ошибка: переменная {name} имеет неподходящий тип({type})");
                    continue;
                }
                if (variableAndType.ContainsKey(name)) Console.WriteLine("переопределение" + " " + $"{type} {name} = {valuable}");
                else
                {
